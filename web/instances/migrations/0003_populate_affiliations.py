...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        instance1 = orm.Instance.objects.all()[0]
        all_aff = orm['accounts.UserProfile'].objects.exclude(affiliations__isnull=True).exclude(affiliations=u'').values_list('affiliations', flat=True).distinct()
        s = set()
        for x in all_aff:
            for y in x.split(','):
                if y.strip() != '':
                    s.add(y.strip().lower())
        for a in s:
            aff, created = orm.Affiliation.objects.get_or_create(name=a)
            if created:
                instance1.affiliations.add(aff)
    ...
    complete_apps = ['accounts', 'instances']
